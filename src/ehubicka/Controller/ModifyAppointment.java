...
/**
 * This class will update an existing appointment record in the database. The form will pre-populate with the
 * data from the record selected (from the mainscreen tableview). Users may or may not change information
 * when updating an appointment. Appointment IDs are a primary key and are not editable. Once the appointment
 * is saved, the user will be taken to the mainscreen.
 */
...
//        appointmentContact.getSelectionModel().selectFirst();
    ...
    //lambda
    /**
     * This will pre-populate the textfields, date-pickers, and combo boxes in the form with the information stored
     * in the database based on which appointment record was selected to be updated from the mainscreen appointment
     * tableview.
     * LAMBDA EXPRESSIONS were used to simplify functions which would get all the names of the Users, Customers,
     * and Contacts, and then set them as choices in their respective combo boxes. Instead of just displaying
     * ID's for these parameters, the names are also shown to make the choices easier to understand.
     * @param selectedItem is the appointment selected to be updated from the mainscreen tableview.
     */
    public void sendAppointment(Appointment selectedItem){ //called on MainScreen controller when going from MainScreen to Update Appointment screen
        appointmentId.setText(String.valueOf(selectedItem.getAppointmentId()));
        appointmentTitle.setText(selectedItem.getAppointmentTitle());
        appointmentDescription.setText(selectedItem.getAppointmentDescription());
        appointmentLocation.setText(selectedItem.getAppointmentLocation());
        appointmentType.setText(selectedItem.getAppointmentType());
        startTime.setValue(selectedItem.getStartTime());
        endTime.setValue(selectedItem.getEndTime());
        startDate.setValue(selectedItem.getStartDate());
        endDate.setValue(selectedItem.getStartDate());

        ObservableList<Contact> contacts = ContactDAO.getAllContacts();
        contacts.forEach(contact -> { //lambda
            if (contact.getContactName().equals(selectedItem.getContactName()))
                appointmentContact.setValue(contact);
        });

        ObservableList<Customer> customers = CustomerDAO.getAllCustomers();
        customers.forEach(customer -> { //lambda
            if (customer.getCustomerName().equals(selectedItem.getCustomerName()))
                customerId.setValue(customer);
        });

        ObservableList<User> users = UserDAO.getAllUsers();
        users.forEach(user -> { //lambda
            if (user.getUserName().equals(selectedItem.getUserName()))
                userId.setValue(user);
        });
    }

    /**
     * Users will be shown various fields of information that can be changed. The data from the selected appointment
     * (from the mainscreen) will be pre-populated in the various fields on the form. Validation will
     * include checks to ensure that appointment times fall within business hours and that multiple appointments
     * from the same customer do not overlap. Once the users saves the information as an updated record, it will be
...
//        System.out.println(hoursDifference + " hours difference.");
        ...
        for (Appointment appointment : appointmentList) { //iterate through each appointment (Appointment object) in the appointmentList
            ...
            //exclude the appointment being modified based on the appointment ID and ensure that the error checking applies only for the same customer
            if (appointment.getAppointmentId() != appointmentId && appointment.getCustomerId() == customer_Id.getCustomerId()) {
                ...
                if (endDateTime.isBefore(startDateTime)){ //if the end date/time comes before the start date/time
                    ...
                    if (duration.toHours() >= 10) {
                        ...
                        if(title.isEmpty() || description.isEmpty() || location.isEmpty() || type.isEmpty()) { //ensure form doesn't have empty fields
                            ...
                            AppointmentDAO.update(appointmentId, title, description, location, type, startDateTime, endDateTime, customer_Id.getCustomerId(), user_Id.getUserId(), contact_Id.getContactId());
    ...
    /**
     * Discard the changes to the appointment to be modified and takes the user back to the mainscreen.
